...
import { ArrowLeft, Lock, Loader2, Sparkles, ShieldAlert } from "lucide-react"
...
export default function AdminLogin() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden bg-background transition-colors duration-300">
      ...
      {/* Dynamic Background Elements - Distinct from User Login (Warmer tones for Admin) */}
      <div className="fixed inset-0 z-0 pointer-events-none">
        <div className="absolute top-[-10%] left-[-5%] w-[500px] h-[500px] rounded-full bg-orange-500/10 blur-[100px] animate-pulse" />
        <div className="absolute bottom-[-10%] right-[-5%] w-[500px] h-[500px] rounded-full bg-red-500/10 blur-[100px] animate-pulse delay-1000" />
      </div>
      ...
      <Card className="w-full max-w-md border-0 shadow-2xl bg-card/40 backdrop-blur-xl relative z-10 overflow-hidden ring-1 ring-white/10 dark:ring-white/5 animate-in zoom-in-95 duration-500">
        {/* Decorative Top Highlight - Admin uses Red/Orange gradient */}
        ...
        <form onSubmit={handleLogin}>
          <CardContent className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="relative group">
                <div className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground group-focus-within:text-orange-500 transition-colors">
                ...
                <Input
                  ...
                  required
                  className="pl-10 h-12 bg-background/50 border-input/50 focus:border-orange-500/50 focus:ring-orange-500/20 transition-all font-mono text-center tracking-wide placeholder:tracking-widest placeholder:text-muted-foreground/50"
          ...
          <CardFooter className="pb-8 pt-2">
            <Button 
              type="submit" 
              className="w-full h-12 text-base font-medium shadow-lg shadow-orange-500/25 transition-all hover:scale-[1.02] active:scale-[0.98] bg-gradient-to-r from-orange-500 to-red-600 hover:from-orange-600 hover:to-red-700 text-white border-0" 
              disabled={loading}
              size="lg"
            >
              {loading ? (
